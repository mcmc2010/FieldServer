...
{
    /// <summary>离开指定房间并通知其他成员；同时退出该房对战（对战中按阵亡处理）并清理位置。</summary>
    public static void LeaveRoom(IRoomManager rooms, IClientConnection connection, int roomId, bool notify = true,
        IBattleManager? battles = null, IMovementManager? movements = null)
    {
        battles?.GetBattle(roomId)?.Leave(connection.Id);
        movements?.GetMovement(roomId)?.Remove(connection.Id);
