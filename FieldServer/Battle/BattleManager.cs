...
public sealed class BattleManager : IBattleManager
{
    ...
    public BattleManager(IOptions<FieldServerOptions> options, ILoggerFactory loggerFactory)
    {
    ...
    public IBattle GetOrCreateBattle(int roomId) =>
        _battles.GetOrAdd(roomId, id => new Battle(id, _options, _loggerFactory.CreateLogger<Battle>()));
